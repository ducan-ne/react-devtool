...
const PropertyElement = ({
  ...
  return (
    <div ref={refElement} className="react-devtool-property">
      <div className="react-devtool-property-content">
        {isExpandableValue && (
          <button type="button" onClick={handleToggleExpand} className="react-devtool-arrow">
            <Icon name="icon-chevron-right" size={12} className={cn(isExpanded && "rotate-90")} />
          </button>
        )}
        ...
        >
          <div className="react-devtool-key !text-white z-[99999] font-display">{name}:</div>
          {isEditing && isEditableValue(value, parentPath) ? (
            <EditableValue value={value} onSave={handleSave} onCancel={() => setIsEditing(false)} />
        ...
        <div className={cn("react-devtool-expandable", isExpanded && "react-devtool-expanded")}>
          {isExpandableValue && isExpanded && (
            <div className="react-devtool-nested">{renderNestedProperties(value)}</div>
          )}
        </div>
      </div>
    </div>
...
export const PropertySection = ({ refSticky, isSticky, name, data }: PropertySectionProps) => {
  ...
  if (
    !currentData ||
    (Array.isArray(currentData) ? currentData.length === 0 : Object.keys(currentData).length === 0)
  ) {
    return null
  }
