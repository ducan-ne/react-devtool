...
import { type Fiber, detectReactBuildType, getRDTHook, isInstrumentationActive } from "bippy"
...
import { setDefaultWidgetCorner } from "~web/state"
import type { States } from "~web/views/inspector/utils"
import type { Corner } from "~web/widget/types"
...
//   /**
//    * Enable/disable render monitoring
//    *
...
//   /**
//    * Force React Devtool to run in production (not recommended)
//    *
//    * @default false
//    */
...
//    * Show toolbar bar
//    *
//    * If you set this to true, and set {@link enabled} to false, the toolbar will still show, but render monitoring will be disabled.
//    *
//    * @default true
//    */
//   showToolbar?: boolean;
...
interface Options {
   ...
   *
   *  @default false
   *  @warning tracking unnecessary renders can add meaningful overhead to react-devtool
   */
  ...
  /**
   * Allow React Devtool to run inside iframes
   *
   * @default false
   */
  ...
  /**
   * Should react devtool log internal errors to the console.
   *
   * Useful if react devtool is not behaving expected and you want to provide information to maintainers when submitting an issue https://github.com/ducan-ne/react-devtool/issues
   *
   *  @default false
   */
  ...
  /**
   * Initial toolbar placement when the user has not moved it yet.
   *
...
type LocalStorageOptions = Omit<
  Options,
  "onCommitStart" | "onRender" | "onCommitFinish" | "onPaintStart" | "onPaintFinish"
>
...
const validateOptions = (options: Partial<Options>): Partial<Options> => {
  ...
  for (const key in options) {
    ...
    switch (key) {
      ...
      case "toolbarPlacement":
        if (!["top-left", "top-right", "bottom-left", "bottom-right"].includes(value as string)) {
          errors.push(`- Invalid toolbar placement "${value}". Using default "bottom-right"`)
        } else {
          validOptions.toolbarPlacement = value as Corner
        }
        break
      case "animationSpeed":
  ...
  if (errors.length > 0) {
    ...
    console.warn(`[React Devtool] Invalid options:\n${errors.join("\n")}`)
  }
...
export const setOptions = (userOptions: Partial<Options>) => {
  try {
    ...
    if (validOptions.toolbarPlacement) {
      setDefaultWidgetCorner(validOptions.toolbarPlacement)
    }
...
const start = () => {
  try {
    ...
    const localStorageOptions = readLocalStorage<LocalStorageOptions>("react-devtool-options")
    ...
    if (!Store.monitor.value && IS_CLIENT) {
      setTimeout(() => {
        ...
        // biome-ignore lint/suspicious/noConsole: Intended debug output
        console.error("[React Devtool] Failed to load. Must import React Devtool before React runs.")
      }, 5000)
    }
  } catch (e) {
    if (ReactDevtoolInternals.options.value._debug === "verbose") {
 ...
 * @public
 */
export const initDevtool = (options: Options = {}) => {
  setOptions(options)
